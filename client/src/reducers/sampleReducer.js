import {
  ...
  CLOSE_NEW_SAMPLE_MODAL, SHOW_EDIT_SAMPLE_MODAL, CLOSE_EDIT_SAMPLE_MODAL
...
export default function samples (state = initialState.samples, action) {
  ...
  switch (action.type) {
    ...
    case CREATE_SAMPLE_SUCCESS:
      console.log(action.type, action.payload)
      return {...state, newSample: {...state.newSample, sample: action.payload, error: null, loading: false}}

    case CREATE_SAMPLE_FAILURE:
      console.log(action.type, action.payload)
      error = action.payload || {message: action.payload.message} // 2nd one is network or server down errors
      return {...state, newSample: {...state.newSample, sample: null, error, loading: false}}

    case RESET_NEW_SAMPLE:
      console.log(action.type, action.payload)
      return {...state, newSample: {sample: null, error: null, loading: false, open: false}}

    case SELECT_SAMPLE_TO_UPDATE:
      console.log(action.type, action.payload)
      return {...state, updatedSample: {...state.updatedSample, sample: action.payload, error: null, loading: false, updated: false}}

    case SHOW_EDIT_SAMPLE_MODAL:
      console.log(action.type, action.payload)
      return {...state, updatedSample: {sample: action.payload, error: null, loading: false, updated: false, open: true}}

    case CLOSE_EDIT_SAMPLE_MODAL:
      console.log(action.type, action.payload)
      return {...state, updatedSample: {...state.updatedSample, open: false}}
    ...
    case UPDATE_SAMPLE_SUCCESS:
      console.log(action.type, action.payload)
      return {...state, updatedSample: {...state.updatedSample, sample: action.payload, error: null, loading: false, updated: true}}

    case UPDATE_SAMPLE_FAILURE:
      console.log(action.type, action.payload)
      error = action.payload || {message: action.payload.message} // 2nd one is network or server down errors
      return {...state, updatedSample: {...state.updatedSample, sample: null, error, loading: false, updated: false}}

    case RESET_UPDATED_SAMPLE:
      console.log(action.type, action.payload)
      return {...state, updatedSample: {sample: null, error: null, loading: false, updated: false, open: false}}
