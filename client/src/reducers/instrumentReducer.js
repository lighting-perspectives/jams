...
export default function instrumentReducer (state = initialState.instruments, action) {
  ...
  switch (action.type) {
    ...
    case CREATE_INSTRUMENT_CLOSE_MODAL:
      return {...state, newInstrument: {...state.newInstrument, open: false}}

    case CREATE_INSTRUMENT:
      return {...state, newInstrument: {...state.newInstrument, loading: true}}

    case CREATE_INSTRUMENT_SUCCESS:
      return {...state, newInstrument: {...state.newInstrument, instrument: action.payload, error: null, loading: false}}

    case CREATE_INSTRUMENT_FAILURE:
      error = action.payload || {message: action.payload.message} // 2nd one is network or server down errors
      return {...state, newInstrument: {...state.newInstrument, instrument: null, error, loading: false}}

    case RESET_NEW_INSTRUMENT:
      return {...state, newInstrument: {instrument: null, error: null, loading: false, open: false}}

    case UPDATE_INSTRUMENT_OPEN_MODAL:
      return {...state, updatedInstrument: {...state.updatedInstrument, instrument: action.payload, open: true}}

    case UPDATE_INSTRUMENT_CLOSE_MODAL:
      return {...state, updatedInstrument: {...state.updatedInstrument, open: false}}

    case UPDATE_INSTRUMENT:
      return {...state, updatedInstrument: {...state.updatedInstrument, loading: true}}

    case UPDATE_INSTRUMENT_SUCCESS:
      return {...state, updatedInstrument: {...state.updatedInstrument, instrument: action.payload, error: null, loading: false, updated: true}}

    case UPDATE_INSTRUMENT_FAILURE:
      error = action.payload || {message: action.payload.message} // 2nd one is network or server down errors
      return {...state, updatedInstrument: {...state.updatedInstrument, instrument: null, error, loading: false, updated: false}}
    ...
    case NEW_MAPPING_CLOSE_MODAL:
      return {...state, newMapping: {...state.newMapping, instrumentId: action.payload, open: false}}

    case CREATE_MAPPING:
      return {...state, newMapping: {...state.newMapping, loading: true}}

    case CREATE_MAPPING_SUCCESS:
      return {...state, newMapping: {...state.newMapping, instrumentId: action.payload.instrumentId, mapping: action.payload.mapping, error: null, loading: false}}

    case CREATE_MAPPING_FAILURE:
      error = action.payload || {message: action.payload.message} // 2nd one is network or server down errors
      return {...state, newMapping: {...state.newMapping, mapping: null, error, loading: false}}

    case RESET_NEW_MAPPING:
      return {...state, newMapping: {instrumentId: null, mapping: null, error: null, loading: false, open: false}}

    case DELETE_MAPPING_OPEN_CONFIRM:
      return {...state, deletedMapping: {...state.deletedMapping, mapping: action.payload, open: true}}
